...
def histweighteddrac(data,weight):
    binsize = 2*sp.stats.iqr(data)/pow(len(data),1/3)
    binn = np.arange(min(data),max(data)+binsize,binsize)
    pyplot.hist(data,bins=binn, weights = weight )

pyplot.figure(0)
pyplot.hist(signal.allcombinedinvariantmass, bins=np.arange(min(signal.allcombinedinvariantmass),max(signal.allcombinedinvariantmass)+(1+np.log2(len(signal.allcombinedinvariantmass))),(1+np.log2(len(signal.allcombinedinvariantmass)))), weights = signal.weight)
...
for x,y in zip(signal.label.unique(),range(len(signal.label.unique())-1)):
    ...
    pyplot.figure(9+3*y)
    pyplot.hist(signalplot.allcombinedinvariantmass, bins=200, weights = signalplot.weight)
    pyplot.ylabel('Frequency')
    pyplot.xlabel('Combined Mass All / %s' % x)
    pyplot.savefig('testallfull%s.png' % x)
    pyplot.figure(10+3*y)
    pyplot.hist(signalplot.lepcombinedinvariantmass, bins=200, weights = signalplot.weight)
    pyplot.ylabel('Frequency')
    pyplot.xlabel('Combined Mass lep / %s' % x)
    pyplot.savefig('testJetfull%s.png' % x)
    pyplot.figure(11+3*y)
    pyplot.hist(signalplot.jetcombinedinvariantmass, bins=200, weights = signalplot.weight)
    pyplot.ylabel('Frequency')
    pyplot.xlabel('Combined Mass jet / %s' % x)
    pyplot.savefig('testLepfull%s.png' % x)